...
public class Ls9Plugin implements IMixerPlugin {
	private final AbstractApplicationContext applContext;
	public final AbstractApplicationContext getApplContext() {
	...
	private static final SLogger LOG = StereoscopeLogManager.getLogger(Ls9Plugin.class);
	private static final String ID = "stereoscope.mixer.yamaha.ls9";

	public Ls9Plugin(final AbstractApplicationContext ctx, final IMidiInputPort inputPort, final IMidiOutputPort outputPort) throws MidiException {
		...
		this.mixer = new Ls9Mixer(this, new ISendMidi() {
			@Override
			public void sendSysexData(final byte[] sysexdata) {
				try {
					Ls9Plugin.this.output.sendSysexMessage(sysexdata);
		...
		final Ls9MidiReceiver receiver = new Ls9MidiReceiver(this.mixer);
	...
	public final IPersistentPluginConfiguration getPersistableConfiguration() {
		return new IPersistentPluginConfiguration() {
			...
			public MixerConnectionBean getConnectionBean() {
				...
				midiConnection.setInputPortName(Ls9Plugin.this.input.getName());
				midiConnection.setOutputPortName(Ls9Plugin.this.output.getName());
