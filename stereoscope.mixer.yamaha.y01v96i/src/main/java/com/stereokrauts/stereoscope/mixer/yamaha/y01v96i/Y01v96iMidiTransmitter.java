 ...
 *
 */
final class Y01v96iMidiTransmitter extends GenericMidsizeMidiTransmitter {
	private final Y01v96iMixer mixer;


	// just to implement the super constructor
	public Y01v96iMidiTransmitter(final Y01v96iMixer mixer, final ISendMidi midi) {
		super(mixer, midi);
		this.mixer = mixer;
	}
	...
	public void requestChannelName(final byte channel) {
		...
		final FormatStringSysex fss = new FormatStringSysex("F0 43 30 3e 1A 02 04 {b} {b} F7");
		for (byte i = 0; i < 4; i++) {
	...
	public void requestSetupParameter(final byte element, final byte parameter,	final byte channel) {
		this.mixer.newMessageToMixer();
		final FormatStringSysex fss = new FormatStringSysex("F0 43 30 3E 1A 03 {b} {b} {b} F7");
	...
	public void requestStereoChannelName(final byte stereoNumber) {
		this.mixer.newMessageToMixer();
		final FormatStringSysex fss = new FormatStringSysex("F0 43 30 3e 1A 02 17 {b} {b} F7");
		for (byte i = 0; i < 4; i++) {
