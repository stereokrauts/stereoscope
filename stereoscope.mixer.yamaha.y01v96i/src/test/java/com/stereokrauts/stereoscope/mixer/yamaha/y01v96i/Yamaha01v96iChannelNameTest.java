...
public class Yamaha01v96iChannelNameTest {
	private DummyMidiSink sink;
	private Y01v96iMixer mixer;

	@Before
	public void initData()
	{
		this.sink = new DummyMidiSink();
		this.mixer = new Y01v96iMixer(null, this.sink);
	}
	...
	public void channelNameRequest()
	{
		...
		final Y01v96iMidiReceiver recv = new Y01v96iMidiReceiver(this.mixer);
		AbstractMessage<?> lastMsg;
		MsgChannelNameChanged expected;

		recv.handleSysex(ByteStringConversion.toBytes(normalize("F0	43	10	3E	1A	02	04	00	00	00	00	00	41	F7")));
		...
		recv.handleSysex(ByteStringConversion.toBytes(normalize("F0	43	10	3E	1A	02	04	01	00	00	00	00	42	F7")));
		lastMsg = sink.getLastMessage();
		expected = new MsgChannelNameChanged(0, "AB01");
		assertFieldsEqual("Channel names should match", expected, lastMsg);

		recv.handleSysex(ByteStringConversion.toBytes(normalize("F0	43	10	3E	1A	02	04	02	00	00	00	00	43	F7")));
		lastMsg = sink.getLastMessage();
		expected = new MsgChannelNameChanged(0, "ABC1");
		assertFieldsEqual("Channel names should match", expected, lastMsg);

		recv.handleSysex(ByteStringConversion.toBytes(normalize("F0	43	10	3E	1A	02	04	03	00	00	00	00	44	F7")));
		lastMsg = sink.getLastMessage();
		expected = new MsgChannelNameChanged(0, "ABCD");
		assertFieldsEqual("Channel names should match", expected, lastMsg);

		/* Test Stereo returns */
		recv.handleSysex(ByteStringConversion.toBytes(normalize("F0	43	10	3E	1A	02	17	00	00	00	00	00	41	F7")));
		lastMsg = sink.getLastMessage();
