 ...
 * degrees/radians are reflected because y-axis coordinates
 * go down (rather than up in a normal coordinate-system).
 */
sk.stsc.view.RotaryStdView = sk.stsc.view.ControlView.extend({
	...
	initialize: function(options) {
		...
		_.bindAll(this, 'handleMouseDown', 'handleMouseMove', 'render', 'updateNeedle'/*, 'drawLevelNeedle'*/);
	...
	setupRotary: function(orient) {
		...
		this.setHitArea(this.container['interactive']);
	...
	/**
	 * This one is tricky:
	 * In order to get a completely transparent hit area
	 * we use a shape without graphics and append another
	 * shape with graphics to it which acts as the actual hit area.
	 */
	setHitArea: function() {
		var ha = new createjs.Shape();
		this.hitArea.hitArea = ha.graphics.beginFill('#f00').rect(
				0, 0,
	...
	drawColorRingBg: function() {
		var rgb = statics.convertHexToRgb(statics.color.GRAY);
		...
		this.colorRingBg.graphics.beginFill(color)
			...
			.endFill().beginFill(statics.color.ui.layoutBg)
		...
		this.rotaryKnob.graphics.beginFill('rgba(0, 0, 0, 1)')
			...
			.beginLinearGradientFill(['rgba(240,240,240,1)', 'rgba(20,20,20,1)'],
				...
				.beginLinearGradientFill(['rgba(130,130,130,1)', 'rgba(80,80,80,1)'],
						[0, .8], this.centerX, 0, this.centerX, this.model.get('height'))
