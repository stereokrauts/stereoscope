...
public class SizeFileComparator extends AbstractFileComparator implements Serializable {
	...
	public int compare(final File file1, final File file2) {
        ...
        if (file1.isDirectory()) {
            size1 = this.sumDirectoryContents && file1.exists() ? FileUtils.sizeOfDirectory(file1) : 0;
        } else {
            size1 = file1.length();
        }
        long size2 = 0;
        if (file2.isDirectory()) {
            size2 = this.sumDirectoryContents && file2.exists() ? FileUtils.sizeOfDirectory(file2) : 0;
        } else {
            size2 = file2.length();
        }
